...
class EClass(str, enum.Enum):
    ...
    def color(self):
        return {
            "Error": QtGui.QColor(Qt.QColorConstants.red),
            "Warning": QtGui.QColor(229, 103, 23),
            "Log": QtGui.QColor(Qt.QColorConstants.darkGreen),
            "Info": QtGui.QColor(Qt.QColorConstants.darkGreen),
            "QA": QtGui.QColor(Qt.QColorConstants.darkGreen),
        }[self.name]
...
class ReadFontStyleDelegate(QtWidgets.QStyledItemDelegate):
    def paint(self, painter, option, index):
        if not index.isValid():
            return
        self.initStyleOption(option, index)
        option.font.setBold(_itemFromIndex(index).readState() == Qt.CheckState.Unchecked)
        super().paint(painter, option, index)
...
class Bullet(QtWidgets.QAbstractButton):
    ...
    def paintEvent(self, event):
        ...
        green = QtGui.QBrush(Qt.QColorConstants.darkGreen)
...
class Star(QtWidgets.QAbstractButton):
    ...
    def paintEvent(self, event):
        ...
        painter.setRenderHint(QtGui.QPainter.Antialiasing, True)
        painter.setPen(Qt.PenStyle.NoPen)
        red = QtGui.QBrush(Qt.QColorConstants.red)
...
class ButtonDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        self._btn.setChecked(index.data(role=Qt.ItemDataRole.CheckStateRole))
    ...
    def editorEvent(self, event, model, _option, index):
        if (
            int(index.flags()) & Qt.ItemFlag.ItemIsEditable
            and (
                event.type()
...
class ElogItem:
    def __init__(self, elog, readState=Qt.CheckState.Unchecked, importantState=Qt.CheckState.Unchecked):
        self._elog = elog
    ...
    def toggleReadState(self):
        self.setReadState(Qt.CheckState.Unchecked if self.isReadState() else Qt.CheckState.Checked)
    ...
    def toggleImportantState(self):
        self.setImportantState(Qt.CheckState.Unchecked if self.isImportantState() else Qt.CheckState.Checked)
...
class Model(QtCore.QAbstractTableModel):
    ...
    def toggleImportantState(self, index):
        return self.setImportantState(
            index,
            Qt.CheckState.Unchecked if self.importantState(index) is Qt.CheckState.Checked else Qt.CheckState.Checked,
        )
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Role.SortRole:
            ...
            }.get(index.column(), lambda: self.data(index, Qt.ItemDataRole.DisplayRole))()
...
class ElogviewerUi(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        horizontalHeader.setSectionResizeMode(horizontalHeader.ResizeMode.ResizeToContents)
...
class Elogviewer(ElogviewerUi):
    def __init__(self, config):
        ...
        if self.settings.contains("windowWidth") and self.settings.contains(
            ...
            screenSize = QtWidgets.QApplication.primaryScreen().availableGeometry() # FIXME properly
        ...
        self.searchLineEdit.textEdited.connect(self.proxyModel.setFilterRegularExpression)
        ...
        if self.settings.contains("sortColumn") and self.settings.contains("sortOrder"):
            self.tableView.sortByColumn(
                ...
                Qt.SortOrder.DescendingOrder if self.settings.value("sortOrder") == 1 else Qt.SortOrder.AscendingOrder
            )
        else:
